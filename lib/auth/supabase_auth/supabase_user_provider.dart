...
class HeritageHuesSupabaseUser extends BaseAuthUser {
  HeritageHuesSupabaseUser(this.user);
  User? user;
...
Stream<BaseAuthUser> heritageHuesSupabaseUserStream() {
  ...
  return (!loggedIn
      ...
      .map<BaseAuthUser>(
    (authState) {
      currentUser = HeritageHuesSupabaseUser(authState?.session?.user);
      return currentUser!;
    },
  );
}
